// Command Jokeapi-cli is a CLI + REPL for the Jokeapi SDK,
...
const prompt = "jokeapi"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "info joke submit"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      JOKEAPI_APIKEY (api key), JOKEAPI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: JOKEAPI_APIKEY carries the API key and
	// JOKEAPI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("JOKEAPI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("JOKEAPI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewJokeapiSDK(opts)
