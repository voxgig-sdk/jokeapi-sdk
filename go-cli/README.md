...
boru-driven command-line client **and** interactive REPL for the Jokeapi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jokeapi-cli)
make build
...
./jokeapi-cli list info
./jokeapi-cli list joke

# 5. Override the API base URL for a single call
JOKEAPI_BASE=https://api.example.com ./jokeapi-cli list info
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jokeapi-cli
   ```
...
Run with no arguments to open a REPL (prompt `jokeapi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `info`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/jokeapi-cli`. |
...
(`./jokeapi-cli <expr>`) and interactively (the REPL), and expressions compose the
