...
class JokeapiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JokeapiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JokeapiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JokeapiTestRunner.getenv("JOKEAPI_TEST_LIVE")
        override = JokeapiTestRunner.getenv("JOKEAPI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JokeapiTestRunner.getenv(key)
        ...
        explain = JokeapiTestRunner.getenv("JOKEAPI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["JOKEAPI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if JokeapiTestRunner._test_control is not None:
            return JokeapiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JokeapiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JokeapiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JokeapiTestRunner.load_test_control()
...
def load_env_local():
    JokeapiTestRunner.load_env_local()


def env_override(m):
    return JokeapiTestRunner.env_override(m)


def entity_data(v):
    return JokeapiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JokeapiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JokeapiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JokeapiTestRunner.load_test_control()


def live_client_options():
    return JokeapiTestRunner.live_client_options()


def live_delay_ms():
    return JokeapiTestRunner.live_delay_ms()
