...
The Python SDK for the Jokeapi API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Info()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/jokeapi-sdk/releases)) or
...
```python
client = JokeapiSDK.test(testopts, sdkopts)
```
...
| --- | --- | --- |
| `category` | `str` |  |
| `delivery` | `str` | The delivery/punchline (for twopart jokes only) |
| `error` | `bool` |  |
| `flags` | `dict` |  |
| `formatVersion` | `int` |  |
| `joke` | `str` | The joke content (for single-type jokes only) |
| `lang` | `str` | Language code (ISO 639-1) |
| `message` | `str` |  |
| `setup` | `str` | The setup part (for twopart jokes only) |
...
```
py/
├── jokeapi_sdk.py         -- Main SDK module
...
The main module (`jokeapi_sdk`) exports the SDK class.
...
# info.data_get() now returns the info data from the last list
# info.match_get() returns the last match criteria
```
