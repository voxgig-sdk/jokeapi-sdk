...
The Python SDK for the Jokeapi API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Info(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Jokeapi(None).load(
    {"id": "test01"}, None
)
