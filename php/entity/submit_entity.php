...
class SubmitEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new SubmitEntity($this->_client, $opts);
    }

    /**
     * @param Submit|array $args Submit data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = JokeapiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Submit|array The current Submit data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Submit fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = JokeapiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Submit fields).
     */
     ...
     * Create a new Submit.
     *
     * @param SubmitCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed SubmitCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Submit|array The created Submit as an assoc-array at the
     *   SDK boundary; throws JokeapiError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = JokeapiHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
