...
class JokeapiSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = JokeapiHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = JokeapiHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!JokeapiFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, JokeapiFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return JokeapiUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = JokeapiHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = JokeapiHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = JokeapiHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new JokeapiError($op . "_allow",
                "JokeapiSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = JokeapiHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new JokeapiError("graphql_error",
                "JokeapiSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Submit($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_submit === null) {
                $this->_submit = new SubmitEntity($this, null);
            }
            return $this->_submit;
        }
        return new SubmitEntity($this, $data);
    }
