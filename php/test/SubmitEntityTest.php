...
class SubmitEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = JokeapiSDK::test(null, null);
        $ent = $testsdk->Submit(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = submit_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "submit." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set JOKEAPI_TEST_SUBMIT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $submit_ref01_ent = $client->Submit(null);
...
function submit_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/submit/SubmitTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["submit01", "submit02", "submit03"] as $k) {
    ...
    $entid_env_raw = getenv("JOKEAPI_TEST_SUBMIT_ENTID");
    ...
    if ($env["JOKEAPI_TEST_LIVE"] === "TRUE") {
        ...
        $client = new JokeapiSDK(Helpers::to_map($merged_opts));
    }
