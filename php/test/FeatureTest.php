...
class FtCtrl extends JokeapiControl
{
...
class FtClient
{
    ...
    private JokeapiUtility $utility;

    public function __construct(JokeapiUtility $utility, array $options)
    {
    ...
    public function get_utility(): JokeapiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JokeapiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new JokeapiUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (JokeapiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JokeapiConfig::make_feature($name);
    ...
    private static function buildUrl(JokeapiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JokeapiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JokeapiResponse($fetched);
        }
    ...
    private function fail(JokeapiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JokeapiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
