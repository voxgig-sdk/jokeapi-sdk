...
local SubmitEntity = {}
SubmitEntity.__index = SubmitEntity


function SubmitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SubmitEntity)
  self._name = "submit"
  self._client = client
...
function SubmitEntity:get_name()
  return self._name
end


function SubmitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SubmitEntity.new(self._client, opts)
end
...
function SubmitEntity:mark_deleted()
  self._deleted = true
end


function SubmitEntity:deleted()
  return true == self._deleted
end


function SubmitEntity:data_set(args)
  if args ~= nil then
...
function SubmitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SubmitEntity:match_set(args)
  if args ~= nil then
...
function SubmitEntity:match_get()
...
function SubmitEntity:stream(action, args, callopts)
...
function SubmitEntity:_run_op(ctx, post_done)
  local utility = self._utility
