...
The Lua SDK for the Jokeapi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Info(nil):list(nil, nil)
...
local result, err = client:Jokeapi(nil):load(
  { id = "test01" }, nil
)
