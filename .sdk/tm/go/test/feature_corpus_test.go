...
//
// NewJokeapiSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.JokeapiSDK {
	res, _ := kase["res"].([]any)
...
func fcCandidates(client *sdk.JokeapiSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.JokeapiSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.JokeapiSDK, name string) any {
