...
module JokeapiUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = JokeapiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = JokeapiResponse.new(fetched)
    else
