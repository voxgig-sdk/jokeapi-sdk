...
The Ruby SDK for the Jokeapi API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Info` — with named operations (`list`/`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
raise a `JokeapiError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Jokeapi_sdk.rb       -- Main SDK module
