...
class SubmitEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = submit_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "submit." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set JOKEAPI_TEST_SUBMIT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    submit_ref01_ent = client.Submit(nil)
...
def submit_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "submit", "SubmitTestData.json")
  ...
  entid_env_raw = ENV["JOKEAPI_TEST_SUBMIT_ENTID"]
  ...
  if env["JOKEAPI_TEST_LIVE"] == "TRUE"
    ...
    client = JokeapiSDK.new(Helpers.to_map(merged_opts))
  end
