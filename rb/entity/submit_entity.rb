...
class SubmitEntity
  ...
  def make
    opts = @_entopts.dup
    SubmitEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = JokeapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Submit, Hash] the current Submit data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = JokeapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Submit fields)
  def match_get
  ...
  # Create a new Submit.
  #
  # @param reqdata [SubmitCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Submit, Hash] the created Submit; raises JokeapiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = JokeapiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
