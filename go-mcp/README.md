...
[MCP](https://modelcontextprotocol.io) server exposing the Jokeapi SDK as
two agent tools — `jokeapi_list` and `jokeapi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jokeapi-mcp)
make build

# 2. Provide credentials via the environment
export JOKEAPI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user jokeapi \
  -- /absolute/path/to/jokeapi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./jokeapi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// jokeapi_list: first page of records
{ "entity": "info" }
{ "entity": "info", "query": { } }

// jokeapi_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jokeapi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user jokeapi \
     -- "$PWD"/dist/*/jokeapi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `jokeapi_list` and `jokeapi_load` tools now appear
   in new sessions. Ask the agent to *"list info using jokeapi"*
   and it calls `jokeapi_list` with `{"entity":"info"}`.
...
```sh
export JOKEAPI_APIKEY=sk_live_xxx            # API key
export JOKEAPI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `jokeapi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `jokeapi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `JOKEAPI_APIKEY` | API key sent with every request. |
| `JOKEAPI_BASE` | Optional override of the API base URL. |
...
```sh
./jokeapi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"jokeapi_load","arguments":{"entity":"joke","query":{"id":1}}}}'
```
