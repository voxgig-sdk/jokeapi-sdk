...
MCP server exposing the Jokeapi SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `jokeapi_list` | `entity`, optional `query` map | First page of records as JSON |
| `jokeapi_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `jokeapi_list` and
`jokeapi_load` tools become available in new sessions.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"jokeapi_load","arguments":{"entity":"info","query":{"id":1}}}}'
```
