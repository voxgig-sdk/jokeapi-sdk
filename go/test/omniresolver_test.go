...
import (
	...
	omni "github.com/voxgig-sdk/jokeapi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.JokeapiError); is {
				out := map[string]any{
					"name":    "JokeapiError",
...
func omniCtx(arg any, client *sdk.JokeapiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.JokeapiSDK); is {
			client = live
