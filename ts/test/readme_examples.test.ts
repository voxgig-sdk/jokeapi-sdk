...
//     (\`new JokeapiSDK(...)\` / \`JokeapiSDK.test(...)\` become
//     \`JokeapiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"info":{"test01":{"id":"test01"}},"joke":{"test01":{"id":"test01"}},"submit":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new JokeapiSDK(...)\` and any
// \`JokeapiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+JokeapiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/JokeapiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
